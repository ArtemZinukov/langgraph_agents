...
Асинхронная мультиагентная система (MAS) поддержки клиентов, построенная на базе **LangGraph** и **Pydantic v2**. Система автоматически классифицирует входящие обращения, извлекает сущности, работает с базой знаний по паттерну RAG, а также включает изолированный контур контроля качества (LLM-as-a-Judge) с механизмом саморефлексии (Self-Reflection) и итеративной доработки ответов.
...
4. **Слой Тестирования (`app/tests/`)**: Модульные и интеграционные тесты для проверки стейта графа, переходов между узлами и логики работы декораторов.
